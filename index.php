<?php 
    ...
    if(isset($_POST['submit'])){
        ...
        if($check_details_row == 1){
            ...
            ob_end_clean();
            if ($sqlcount == 1){
                echo json_encode(array("response"=>"Succesfully"));
...
<html class="no-js" lang="">
    <head>
        ...
        <title> REMOTE SGOP EMPLOYEES</title>
 ...
 <body id="loginPage">
 	<div class="login_wrapper clearfix">
 		<div class="logo_login">
             <h1> REMOTE SGOP EMPLOYEES </h2>
 		</div>
        <?php
            if(isset($_GET['error'])){
                if($error_login == "failed_login"){?>

                    <div class="LogResponse2">Firstly autho</div>
        ...
        <div class="LogResponse"></div>
 		<div class="login_wrapper_inner">
 			<form id="loginForm" class="clearfix" method="post" action="">
	 			...
	 			<div class="input-box">
	 				<button  type="submit" class="submitField sign_in"><span class="sign-icon"><i class="fa fa-lock"></i></span> Connect</button>
	 			</div>
	 		</form>
 		</div>
 	</div>
